...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Aldershot gets higher priority)
    const isAldershot = location.slug === 'aldershot';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isAldershot ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isAldershot ? 1 : 2 + i),
    });
