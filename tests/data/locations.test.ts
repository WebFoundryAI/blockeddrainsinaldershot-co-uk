...
describe('Locations Data', () => {
  ...
  describe('PRIMARY_LOCATION', () => {
    it('is "aldershot"', () => {
      expect(PRIMARY_LOCATION.slug).toBe('aldershot');
    });
  ...
  describe('INDEXED_LOCATIONS', () => {
    ...
    it('includes all location slugs', () => {
      ...
      expect(slugs).toContain('aldershot');
      expect(slugs).toContain('farnham');
      expect(slugs).toContain('farnborough');
      expect(slugs).toContain('fleet');
      expect(slugs).toContain('camberley');
      expect(slugs).toContain('guildford');
      expect(slugs).toContain('ash');
      expect(slugs).toContain('yateley');
      expect(slugs).toContain('sandhurst');
    });
  });

  describe('getLocationBySlug', () => {
    it('returns correct location for "aldershot"', () => {
      const loc = getLocationBySlug('aldershot');
      expect(loc).toBeDefined();
      expect(loc!.name).toBe('Aldershot');
    });

    it('returns correct location for "farnham"', () => {
      const loc = getLocationBySlug('farnham');
      expect(loc).toBeDefined();
      expect(loc!.name).toBe('Farnham');
    });
